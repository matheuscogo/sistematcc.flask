...
from ..site.model.Dias import DiasSchema
from ..site.model import Dias
from ..db import db
...
def cadastrarDia(args):  # Create
    try:
        ...
        db.session.add(Dias.Dias(plano=plano,dia=dia,quantidade=quantidade))
        db.session.commit()
...
def consultarDia(id):  # Read
    try:
        dia = db.session.query(Dias.Dias).filter_by(id=id).first()
        if not dia:
            raise Exception("")
